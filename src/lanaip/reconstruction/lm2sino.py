...
def build_sinogram_and_hitmap(lm_path, force_v2=False, force_v1=False):
    ...
    with open(lm_path, "rb") as f:
        ...
        for _ in tqdm(range(n_events), desc="Parsing events"):
            ...
            if ring_dif <= 0:
                bin_axial = int(sum(range(int(num_rings*module_size+(num_rings-1)*ring_gap+ring_dif))) + global_y1)
            else:
                bin_axial = int(bins_axial - (sum(range(int(num_rings*module_size+(num_rings-1)*ring_gap-ring_dif))) + global_y2)) -1
